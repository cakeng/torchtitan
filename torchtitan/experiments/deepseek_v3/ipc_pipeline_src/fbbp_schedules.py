...
class _PipelineSchedule(ABC):
    ...
    def _update_losses(self, stages, losses, microbatch_list = None):
        """
        ...
        microbatch_size = len(microbatch_list) if microbatch_list is not None else self._n_microbatches
        if contains_last_stage and losses is not None:
            if len(self._internal_losses) != microbatch_size:
                raise RuntimeError(
                    f"Expecting {microbatch_size} losses but got {len(self._internal_losses)}"
                )
...
class ScheduleGPipe(PipelineScheduleSingle):
    ...
    def _step_microbatches(
        ...
        for i in range(self._n_microbatches):
            with record_function(f"Forward {i}"):
                ops = self._stage.get_fwd_recv_ops(i)
                logging.info(g_str(f"Rank {rank}: ") + b_str(f"Forward {i}") + f", receiving activations from previous stage: {ops}")
                works = _sorted_batch_p2p(ops, desc="fwd_recv")
                for work in works.values():
                    work.wait()
                logging.info(g_str(f"Rank {rank}: ") + b_str(f"Forward {i}") + f", received activations from previous stage: {ops}")

                output = self._stage.forward_one_chunk(i, arg_mbs[i], kwarg_mbs[i])  # type: ignore[index]

                ops = self._stage.get_fwd_send_ops(i)
                logging.info(g_str(f"Rank {rank}: ") + b_str(f"Forward {i}") + f", sending activations to next stage: {ops}")
                works = _sorted_batch_p2p(ops, desc="fwd_send")
                fwd_sends_to_wait.extend(works.values())
        ...
        for i in range(self._n_microbatches):
            with record_function(f"Backward {i}"):
                ...
                logging.info(g_str(f"Rank {rank}: ") + r_str(f"Backward {i}") + f", receiving activations from next stage: {ops}")
                works = _sorted_batch_p2p(ops, desc="bwd_recv")
                for work in works.values():
                    work.wait()
                logging.info(g_str(f"Rank {rank}: ") + r_str(f"Backward {i}") + f", received activations from next stage: {ops}")
                ...
                logging.info(g_str(f"Rank {rank}: ") + r_str(f"Backward {i}") + f", sending activations from previous stage: {ops}")
...
class ScheduleForwardOnly(PipelineScheduleSingle):
    """
    Forward-only schedule that performs only forward passes through all microbatches.
    Designed to work with ScheduleBackwardOnly for separated forward/backward execution.
    """
    ...
    def __init__(
        ...
        # Override to indicate this is forward-only
        self._stage.has_backward = False
        self.sample_fwd_cache = sample_fwd_cache
        rank = dist.get_rank()
        logging.info(b_str(f"Rank {rank}: ") + "Using ScheduleForwardOnly")
    ...
    def _step_microbatches(
        ...
        """
        Run forward passes for all microbatches.
        """
        arg_mbs, kwarg_mbs = self._check_inputs(arg_mbs, kwarg_mbs, target_mbs, losses)
        ...
        fwd_sends_to_wait: list[dist.Work] = []
        rank = dist.get_rank()
        logging.info(g_str(f"Rank {rank}: ")+ b_str(f"Running Forward!"))

        # Run forward passes for all microbatches
        mb_runs = self._n_microbatches if not self.sample_fwd_cache else 1

        for i in range(mb_runs):
            with record_function(f"Forward {i}"):
                ...
                if not self.sample_fwd_cache:
                    self._maybe_compute_loss(self._stage, output, target_mbs, i)

                    # Send losses to the backward process if this is the last stage
                    work = self._stage._maybe_send_loss(self._stage, i, self._internal_losses)
                    if work is not None:
                        fwd_sends_to_wait.extend(work.values())
        ...
        # Update losses container
        self._update_losses(self._stage, losses)


class ScheduleBackwardOnly(PipelineScheduleSingle):
    """
    Backward-only schedule that performs only backward passes through all microbatches.
    Designed to work with ScheduleForwardOnly for separated forward/backward execution.
    """
    ...
    def __init__(
        ...
        # Ensure stage knows it has backward
        self._stage.has_backward = True
        rank = dist.get_rank()
        self.microbatch_list = []
        for i in range(self._n_microbatches):
            if ((i % (dist.get_world_size(self._stage.fbbp_group) - 1)) 
                    == (dist.get_rank(self._stage.fbbp_group) - 1)):
                self.microbatch_list.append(i)
        logging.info(r_str(f"Rank {rank}: ") + "Using ScheduleBackwardOnly" + 
                     f"- Microbatch List: {self.microbatch_list}")
    ...
    def _step_microbatches(
        ...
        """
        Run backward passes for all microbatches.
        """
        arg_mbs, kwarg_mbs = self._check_inputs(arg_mbs, kwarg_mbs, target_mbs, losses)

        if not self._stage_initialized:
            # Initialize stage infrastructure for backward
            self._initialize_stage(arg_mbs[0], kwarg_mbs[0])
        ...
        logging.info(g_str(f"Rank {rank}: ") + r_str(f"Running Backward "))
                 
        for i, mb_index in enumerate(self.microbatch_list):
            with record_function(f"Backward {mb_index}"):
                # Receive losses from the forward process if this is the last stage
                received_loss = self._stage._maybe_recv_loss(self._stage, mb_index, 
                                                             self._n_microbatches)
                if received_loss is not None:
                    self._internal_losses.append(received_loss)
                    assert len(self._internal_losses) == i + 1, "Received losses out of order"

                # Receive gradients from next stage
                # This will naturally block until forward process completes and sends gradients
                ops = self._stage.get_bwd_recv_ops(mb_index)
                works = _sorted_batch_p2p(ops, desc="bwd_recv")
                for work in works.values():
                    work.wait()
                logging.info(g_str(f"Rank {rank}: ")+ r_str(f"Backward {mb_index}") + 
                             f", received gradients from next stage: {ops}")

                # Get corresponding loss for this microbatch
                loss = self._maybe_get_loss(self._stage, mb_index)
                ...
                ops = self._stage.get_bwd_send_ops(mb_index)
                logging.info(g_str(f"Rank {rank}: ") + r_str(f"Backward {mb_index}") + 
                             f", sending gradients to previous stage: {ops}")
                works = _sorted_batch_p2p(ops, desc="bwd_send")
                bwd_sends_to_wait.extend(works.values())

            logger.debug("[%s] Backwarded microbatch %s", self._stage.stage_index, mb_index)
