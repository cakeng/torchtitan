...
class _TschedStageBase(ABC):
    ...
    def __init__(
        ...
        Args:
            ...
            num_microbatches (int): The total number of microbatches.
    ...
    def _print_total_order_keys(self, debug=False):
        if debug:
            output = g_str(f"[Stage {self.stage_index}] ")
            num_microbatches = self.num_microbatches
            for i in range(num_microbatches * 4):
                target_op = None
                target_mb = None
                for mb in range(num_microbatches):
                    for op in ["fwd_recv", "fwd_send", "bwd_recv", "bwd_send"]:
                        if self.total_order_keys[mb][op] == i:
                            target_op = op
                            target_mb = mb
                            break
                output += f"{target_op} {target_mb}, "
            print(b_str(f"Total Order: ") + f"{output}")
    ...
    def _calculate_total_order_keys(self, debug=False) -> int:
        ...
        for n in range(num_microbatches + num_stages * 2):
            if pp_stage % 2 == 0:
                ...
                bwr_mb = n - num_stages * 2 + pp_stage + 1
                do_bwr_mb = bwr_mb >= 0 and bwr_mb < num_microbatches
                bws_mb = n - num_stages * 2 + pp_stage
...
class TschedStage(_TschedStageBase):
    """
    A class representing a pipeline stage in a pipeline parallelism setup.

    TschedStage assumes sequential partitioning of the model, i.e. the model is split into chunks where outputs from
    one chunk feed into inputs of the next chunk, with no skip connections.

    TschedStage performs runtime shape/dtype inference automatically by propagating the outputs from stage0 to
    stage1 and so forth, in linear order.  To bypass shape inference, pass the `input_args` and `output_args` to each
    TschedStage instance.
    ...
    def __init__(
        ...
        super().__init__(submodule, num_microbatches, microbatch_idx, stage_index, num_stages, 
        ...
        if input_args is None:
            ...
            if output_args is None:
                logger.warning(
                    ...
                    "TschedStage now supports runtime shape inference using the real inputs provided to schedule step(). "
                    "Either delete `input_args` arg to `TschedStage` to opt-into runtime shape inference, "
                    "or additionally pass `output_args` to `TschedStage` to fully override shape inference. "
                )
                try:
                    with torch.no_grad():
