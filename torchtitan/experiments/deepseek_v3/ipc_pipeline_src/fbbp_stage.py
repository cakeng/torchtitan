...
class _PipelineStageBase(ABC):
    ...
    def __init__(
        ...
        do_fbbp: bool = False,
        fbbp_group: Optional[dist.ProcessGroup] = None,
        shared_fwd_cache: Optional[dict] = None,
    ):
        """
        Args:
        ...
        if do_fbbp and shared_fwd_cache is not None:
            self.fwd_cache = shared_fwd_cache
            logging.info(g_str(f"Rank {self.global_rank} using shared forward cache of size {len(self.fwd_cache)}"))
        else:
            logging.info(r_str(f"Rank {self.global_rank} using non-shared forward cache"))
        ...
        # FB pipeline infragroup else 1
        self.do_fbbp = do_fbbp
        self.fbbp_group = fbbp_group
        self.loss_recv_info: Optional[dict] = None
    ...
    def _get_fwd_cache_state_str(self):
        out_str = ""
        for key, (output_tuple, flatten_input_tensors) in self.fwd_cache.items():
            output_tuple_str = [str(t.size()) if isinstance(t, torch.Tensor) 
                                else t for t in output_tuple]
            output_tuple_str = tuple(output_tuple_str)
            flatten_input_tensors_str = [str(t.size()) if isinstance(t, torch.Tensor)
                                            else t for t in flatten_input_tensors]
            flatten_input_tensors_str = tuple(flatten_input_tensors_str)
            out_str += f"{key} - output: {output_tuple_str}, input: {flatten_input_tensors_str}, "
        return out_str
    
    def _get_bwd_process_fbbp_group_rank(self, microbatch_idx) -> int:
        """Calculate the rank of the backward process for this stage."""
        fbbp_size = dist.get_world_size(self.fbbp_group)
        if fbbp_size == 1:
            return FWD_PROCESS_RANK
        return (microbatch_idx % (dist.get_world_size(self.fbbp_group) - 1)) + 1
    
    def _get_fwd_process_fbbp_group_rank(self) -> int: 
        """Calculate the rank of the forward process for this stage."""
        return FWD_PROCESS_RANK
    
    def _create_loss_recv_info(self, num_microbatches: int) -> dict[int, _RecvInfo]:
        """
        Create recv info for losses from forward to backward process.
        Only last stage should receive losses.
        """
        if not self.is_last:
            return {}
        
        fwd_process_rank = self._get_fwd_process_fbbp_group_rank()

        loss_recv_info = {}
        for mb_index in range(num_microbatches):
            # Create receive buffer for loss - assuming scalar loss
            if mb_index % (dist.get_world_size(self.fbbp_group) - 1) == \
                (dist.get_rank(self.fbbp_group) - 1):
                loss_buffer = torch.zeros(1, device=self.device, dtype=torch.float32)
                loss_recv_info[mb_index] = \
                    _RecvInfo(f"loss_recv_for_mb_{mb_index}", fwd_process_rank, loss_buffer)
        
        return loss_recv_info
    
    def _maybe_send_loss(self, stage, mb_index: int, internal_losses: list[torch.Tensor]):
        """
        Send loss from forward process to backward process if this is the last stage.
        Should be called after loss computation in forward process.
        """
        if not stage.is_last:
            return None
        
        backward_process_rank = (mb_index % (dist.get_world_size(self.fbbp_group) - 1)) + 1
        work = None
        
        # Check if we have a loss to send for this microbatch
        if mb_index < len(internal_losses):
            loss = internal_losses[mb_index]
            
            if backward_process_rank is not None:
                # Prepare loss tensor for sending (ensure it's a tensor)
                if not isinstance(loss, torch.Tensor):
                    loss_tensor = torch.tensor(loss, device=self.device)
                else:
                    loss_tensor = loss.detach()
                
                # Ensure loss is a 1D tensor for consistent communication
                loss_tensor = loss_tensor.view(-1)
                
                # Create P2P send operation
                peer_global_rank = dist.get_global_rank(self.fbbp_group, 
                                                        backward_process_rank)
                
                # Send the loss
                send_op = dist.P2POp(dist.isend, loss_tensor, peer_global_rank, self.fbbp_group)
                logging.info(y_str(f"Rank {self.global_rank}: ") + 
                             b_str(f"sending loss ") + f"for microbatch {mb_index} " +
                                   f"to backward process rank: {peer_global_rank}")
                work = {}
                # Execute the send operation
                work[peer_global_rank] = dist.batch_isend_irecv([send_op]).pop()

        return work
    
    def _maybe_recv_loss(self, stage, mb_index: int, num_microbatches: int) -> Optional[torch.Tensor]:
        """
        Receive loss from forward process to backward process if this is the last stage.
        Should be called before backward computation in backward process.
        """
        if not stage.is_last:
            return None
        
        # Initialize loss recv info lazily  
        if self.loss_recv_info is None:
            self.loss_recv_info = self._create_loss_recv_info(num_microbatches)
        
        # Check if we need to receive loss for this microbatch
        if mb_index in self.loss_recv_info:
            recv_info = self.loss_recv_info[mb_index]
            forward_process_rank = recv_info.source

            peer_global_rank = dist.get_global_rank(self.fbbp_group,
                                                    forward_process_rank)
            recv_op = dist.P2POp(dist.irecv, recv_info.buffer, peer_global_rank, self.fbbp_group)
        
            # Execute the recv operation
            work = dist.batch_isend_irecv([recv_op]).pop()
            if work:
                work.wait()
            logging.info(y_str(f"Rank {self.global_rank}: ") + 
                         r_str(f"recieved loss ") + f"for microbatch {mb_index} " +
                               f"from forward process rank: {peer_global_rank}")

            return recv_info.buffer
    ...
    def _create_grad_send_info(
        ...
        logging.info("%s Grad send info: %s", self.log_prefix, grad_send_info)
    ...
    def backward_maybe_with_nosync(
        ...
        def perform_backward(
            ...
            logging.info(g_str(f"Rank {self.global_rank}: ") +
                            b_str(f"Running {backward_type} perform backward, input: {bwd_kwargs}"))
        ...
        if isinstance(self.submod, DistributedDataParallel):
            if last_backward:
                ...
                logging.info(y_str(f"Rank {self.global_rank}: ") + r_str(f"Running DDP last backward"))
                result = perform_backward(backward_type)()
            else:
                logging.info(y_str(f"Rank {self.global_rank}: ") + r_str(f"Running DDP backward"))
                with self.submod.no_sync():  # type: ignore[operator]
                    result = perform_backward(backward_type)()
        # If submod is a FSDP module
            ...
            logging.info(y_str(f"Rank {self.global_rank}: ") + r_str(f"Running FSDP backward"))
            result = perform_backward(backward_type)()
            if last_backward:
            ...
            logging.info(y_str(f"Rank {self.global_rank}: ") + r_str(f"Running regular backward"))
            result = perform_backward(backward_type)()
    ...
    def compiled_forward_one_chunk(
        self,
        fwd_chunk_id: int,
        args: tuple[Any, ...],
        kwargs: Optional[dict[str, Any]] = None,
    ):
        return self.forward_one_chunk(fwd_chunk_id, args, kwargs)

    @torch.compile(mode="max-autotune")
    def compiled_backward_one_chunk(
        self,
        bwd_chunk_id: int,
        loss=None,
        ...
        return self.backward_one_chunk(bwd_chunk_id, loss, full_backward, last_backward)
    ...
    def forward_one_chunk(
        ...
        # If using shared memory, convert all tensors in the cache entry to shared tensors
        if self.do_fbbp:
            # Share output tensors
            cache_idx = fwd_chunk_id % len(self.fwd_cache)
            for i, t in enumerate(output_tuple):
                self.fwd_cache[cache_idx][0][i].copy_(t)
            for i, t in enumerate(flatten_input_tensors):
                self.fwd_cache[cache_idx][1][i].copy_(t)
        else:
            cache_idx = fwd_chunk_id
            self.fwd_cache[cache_idx] = (
                output_tuple,           # stage_output
                flatten_input_tensors,  # input_values
            )
        logging.info(y_str(f"Rank {self.global_rank}: ") + 
                     b_str(f"Stored forward chunk {fwd_chunk_id} in cache {cache_idx}."))
    ...
    def backward_one_chunk(
        ...
        """
        self._check_chunk_id(bwd_chunk_id)
        logging.info(y_str(f"Rank {self.global_rank}: ") + r_str(f"Backwarding chunk {bwd_chunk_id}"))
        if self.do_fbbp:
            cache_idx = bwd_chunk_id % len(self.fwd_cache)
            stage_output, input_values = self.fwd_cache[cache_idx]
        else:
        ...
        if self.dw_builder:
            ...
            logging.info(y_str(f"Rank {self.global_rank}: ") + 
                     r_str(f"Backwarding {bwd_chunk_id} using dw_builder, grads: {grads_input}"))
        else:
            if full_backward:
                grads_input, _ = self.backward_maybe_with_nosync(
                ...
                logging.info(y_str(f"Rank {self.global_rank}: ") + 
                     r_str(f"Backwarding {bwd_chunk_id} using full backward, grads: {grads_input}"))
            else:
                ...
                if not self.is_first:
                    ...
                    logging.info(y_str(f"Rank {self.global_rank}: ") + 
                        r_str(f"Backwarding {bwd_chunk_id} using input backward, grads: {grads_input}"))
        ...
        if self.is_last and not self.is_first and not self.do_fbbp:
        ...
        logging.info(y_str(f"Rank {self.global_rank}: ") + 
                     r_str(f"Backwarded chunk {bwd_chunk_id}, grads: {grads_input}"))
...
class _PipelineStage(_PipelineStageBase):
    ...
    def _move_submod_to_device(self):
        ...
        if has_meta_param:
            logging.debug("%s Found meta parameters!", self.log_prefix)
        else:
    ...
    def _create_act_send_info(self):
        ...
        logging.info("%s Send info: %s", self.log_prefix, act_send_info)
    ...
    def _create_grad_recv_info(
        ...
        logging.info("%s Grad recv info: %s", self.log_prefix, grad_recv_info_tuple)
...
class PipelineStage(_PipelineStageBase):
    ...
    def __init__(
        ...
        super().__init__(submodule, stage_index, num_stages, device, group, dw_builder,
                         global_rank=global_rank, do_fbbp=do_fbbp, fbbp_group=fbbp_group, 
        ...
        dbg_str = (
            g_str(f"Rank {global_rank}: Finished pipeline stage init,") + f" {self.stage_index=}, {self.is_first=}, " +  # noqa: G004
            f"{self.is_last=}, {self.num_stages=}, pipeline group rank={self.group_rank}, " +
            f"fbbp_group_rank={dist.get_rank(self.fbbp_group) if self.do_fbbp else FWD_PROCESS_RANK}, " + 
            f"{self.device=}"
        )
